	<?php 
		...
		$sqls = "SELECT `id`, `title`, `category`, `myfile`, `body` FROM `blog` ORDER BY id DESC LIMIT 9";
        ...
        echo "enter your social media link";

        $userID = '';
        $userMARK = '';

        echo "Are you sure that".$userID."is your ID";

        $userREPLY = '';

        if ($userREPLY=='yes') {
        	echo "Your social boost will start within 24hrs";
        }elseif ($userREPLY = 'no') {
        	echo "You have cancelled your boost request";
        }else{
        	echo " ";
        }
